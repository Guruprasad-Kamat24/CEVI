...
Original file is located at
    https://colab.research.google.com/drive/1IG_GWnC1hMW0OjpAg2yI9-XpKvKPlD9h
"""
...
class LyftUdacity(Dataset):
    def __init__(self,image_dir,depth_dir,norm_dir,seg_dir,transform = None,tm = None):
        self.transforms = transform
        self.tm = tm
        
        self.images,self.depth,self.norm,self.masks = [],[],[],[]
        ...
        for idx in range(0,2975):
            x3 = str(idx)
            temp_r_path = norm_dir + '/' + 'gfg_dummy_pic' + x3 + '.png'
            self.norm.append(temp_r_path)
    ...
    def __getitem__(self,index):
        ...
        # img.resize((256,256,3),refcheck=False)
        # dep = cv2.imread(self.depth[index])
        # dep.resize((256,256,3),refcheck=False)
        # nor = cv2.imread(self.norm[index])
        # nor.resize((256,256,3),refcheck=False)
        # sem = cv2.imread(self.sem[index])
        # sem.resize((256,256,3),refcheck=False)
        

        # cat = np.concatenate((img, dep, nor, sem),axis=2)
        ...
        # print(sem.shape)
        # print(cat.shape)
        # cat = torch.cat((img,dep,nor,sem),dim=2)
        # cat = cat.to(torch.float32)
        # cat = einops.rearrange(cat, 'h w c -> c h w')
        # print(cat.shape)
        # cat = torch.from_numpy(cat)
        #mask = np.array(Image.open(self.masks[index]))
        # mask = cv2.imread(self.masks[index]).astype(np.float32)
        ...
        surf= np.array(Image.open(self.norm[index]))
        
        surf = cv2.resize(surf, dim, interpolation = cv2.INTER_AREA)
        
        
        
        cat = cv2.merge((img,depth,surf))
...
norm_dir="/content/drive/MyDrive/Cityscape/NORMALS"
...
def get_images(image_dir,seg_dir,transform = None,tm = None,batch_size=8,shuffle=True,pin_memory=False):
    data = LyftUdacity(image_dir,depth_dir,norm_dir,seg_dir,transform = t1,tm =tm)
...
train_batch,test_batch = get_images(image_dir,seg_dir,batch_size=8)
...
class unet_model(nn.Module):
    def __init__(self,out_channels=2,features=[64, 128, 256, 512]):
        ...
        self.conv1 = encoding_block(7,features[0])
...
for epoch in range(num_epochs):
    ...
    if(x%2 == 0):
        cur_acc = check_accuracy(test_batch, model)
        if(cur_acc >prev_acc):
            prev_acc = cur_acc
            PATH ="modelconcate"+str(x)+".pth"
